...
export default function Edit({ auth, karyawan, roles, cabangs }) {
    const { data, setData, put, processing, errors } = useForm({
        role_id: karyawan?.role_id || '',
        user_id: karyawan?.user_id || '',
        nig: karyawan?.nig || '',
        posisi: karyawan?.posisi || '',
        status: karyawan?.status || 'Tetap',
    ...
    const [filterCabang, setFilterCabang] = useState(karyawan?.user_cabang_id || '');
    const [filterDepartemen, setFilterDepartemen] = useState(karyawan?.user_departemen_id || '');
    ...
    useEffect(() => {
        if (filterCabang && data.role_id) {
            ...
            const params = {
                ...
                include_user_id: karyawan.user_id // Include current user in list
            };
            if (filterDepartemen) {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Karyawan</h2>}
        >
                    ...
                    <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6">
                            <form onSubmit={submit} className="space-y-6">
                                ...
                                <div>
                                    <InputLabel htmlFor="nig" value="NIG (Nomor Induk Karyawan)" />
                                    <TextInput
                                        id="nig"
                                        type="text"
                                        value={data.nig}
