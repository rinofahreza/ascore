...
export default function Index({ auth, karyawans, cabangs, roles, filters }) {
    ...
    const handleDelete = (id) => {
        router.delete(route('karyawan.destroy', id), {
            onSuccess: () => {
                setConfirmDialog({ isOpen: false, karyawanId: null, karyawanNama: '' });
            }
        });
    };
    ...
    // Apply filters
    const applyFilters = () => {
        router.get(route('karyawan.index'), {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Karyawan</h2>}
        >
            <Head title="Karyawan" />
                    ...
                    <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6">
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Daftar Karyawan</h3>
                                        ...
                                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            <thead className="bg-gray-50 dark:bg-gray-700">
                                                <tr>
                                                    ...
                                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">NIG</th>
                                                ...
                                                {karyawans.data.map((karyawan, index) => (
                                                    <tr key={karyawan.id}>
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                                            {karyawan.user?.cabang?.nama || '-'}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                                            {karyawan.user?.departemen?.nama || '-'}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                                            {karyawan.role?.nama || '-'}
                                                        </td>
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                                            ...
                                                            {hasPermission('karyawan.delete') && (
                                                                <button
                                                                    onClick={() => setConfirmDialog({ isOpen: true, karyawanId: karyawan.id, karyawanNama: karyawan.user?.name || 'Karyawan' })}
                                    ...
                                    {karyawans.last_page > 1 && (
                                        <div className="mt-6 flex items-center justify-between border-t border-gray-200 dark:border-gray-700 pt-4">
                                            <div className="text-sm text-gray-700 dark:text-gray-300">
                                                Menampilkan <span className="font-medium">{karyawans.from}</span> sampai{' '}
                                                <span className="font-medium">{karyawans.to}</span> dari{' '}
                                                <span className="font-medium">{karyawans.total}</span> data
                                            </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
                        ...
                        <p className="text-gray-600 dark:text-gray-400 mb-6">
                            Apakah Anda yakin ingin menghapus karyawan "<strong>{confirmDialog.karyawanNama}</strong>"?
                        </p>
                        <div className="flex justify-end space-x-3">
                            <button
                                onClick={() => setConfirmDialog({ isOpen: false, karyawanId: null, karyawanNama: '' })}
                                className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600"
                            >
                                Batal
                            </button>
                            <button
                                onClick={() => handleDelete(confirmDialog.karyawanId)}
