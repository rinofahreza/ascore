...
export default function Edit({ auth, periodeAkademik }) {
    const { data, setData, put, processing, errors } = useForm({
        nama: periodeAkademik?.nama || '',
        status: periodeAkademik?.status || 'Draft',
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('periode-akademik.update', periodeAkademik.id));
    };

    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Periode Akademik</h2>}
        >
            <Head title="Edit Periode Akademik" />
